...
export async function registrarCompra(formData: FormData) {   
  ...
  if (userError || !user) {     
    return { error: 'Debes iniciar sesi贸n para registrar compras.' };   
  }    
  ...
  if (isNaN(cantidad) || cantidad <= 0) {     
    return { error: 'La cantidad debe ser un n煤mero mayor a 0.' };   
  }   
  if (isNaN(costo_total) || costo_total < 0) {     
    return { error: 'El costo total debe ser un n煤mero v谩lido.' };   
  }    
  ...
  if (!insumo_id) {     
    return { error: 'Debes seleccionar un insumo v谩lido o crear uno nuevo.' };   
  }    
  // 4. Ejecutar RPC para la transacci贸n at贸mica   
  ...
  if (rpcError) {     
    ...
    return { error: 'Ocurri贸 un error al registrar la compra. Intenta de nuevo.' };   
  }    
