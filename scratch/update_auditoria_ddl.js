...
async function run() {
  // Primero creamos exec_ddl usando exec_sql, el cual puede retornar records vacios
  await supabase.rpc('exec_sql', { query: createExecDDL });
  
  // Luego corremos el DDL usando el nuevo RPC
  const { error } = await supabase.rpc('exec_ddl', { query: ddl });
