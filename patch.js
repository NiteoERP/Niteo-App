...
// 2. Replace prod
const prodSearch = "const nombre = prompt('Ingresa el nombre del nuevo producto base:');";
const prodOld1 = "if (nombre && nombre.trim()) {";
const prodOld2 = "crearProductoBase(nombre.trim()).then(res => {";
const prodReplace = "openPrompt('Crear Nuevo Producto Base', 'Ej: Cemento Gris', (nombre) => {\n        if (nombre && nombre.trim()) {\n          setIsSubmitting(true);\n          crearProductoBase(nombre.trim()).then(res => {";

let temp = code.replace(prodSearch, "");
temp = temp.replace(prodOld1, prodReplace);
temp = temp.replace("          setIsSubmitting(false);\n        });\n      }\n      return;", "          setIsSubmitting(false);\n        });\n      }\n      });\n      return;");
code = temp;

// 3. Replace prov
const provSearch = "const handleCrearProveedorRapido = async () => {\n    const nombre = prompt('Ingresa el nombre del nuevo proveedor (y RIF opcional separados por";
const regexProv = /const handleCrearProveedorRapido = async \(\) => {[\s\S]*?setIsSubmitting\(false\);\n      }\n    };/;
...
code = code.replace(regexProv, provReplaceFinal);
