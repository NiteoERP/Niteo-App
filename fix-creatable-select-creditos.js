...
if (!code.includes('react-select/creatable')) {
  code = code.replace(
...
const selectGlobalRegex = /<select[\s\S]*?value=\{metodoPago\}[\s\S]*?onChange=\{\(e\) => setMetodoPago\(e\.target\.value\)\}[\s\S]*?className="w-full bg-neutral-950 border border-neutral-800 focus:border-neutral-600 text-white font-medium py-3 px-4 rounded-xl outline-none appearance-none"[\s\S]*?>[\s\S]*?\{metodosDisponibles\.map\(m => \([\s\S]*?<option key=\{m\} value=\{m\}>\{m\}<\/option>[\s\S]*?\)\)\}[\s\S]*?<\/select>/g;

code = code.replace(selectGlobalRegex, `<CreatableSelect
